...
$booking_com_default_printed = false;
...
<div data-rating="<?php echo esc_attr($rating); ?>" class="wpsr-rating-wrapper <?php echo esc_attr($rating_class); ?>">
    <?php if($rating_style === 'default' && $platform_name === 'ai' && $platforms_array && count($platforms_array) === 1 && in_array('booking.com', $platforms_array)) {
        $booking_com_default_printed = true;
        ?>
    ...
    <?php if (!$booking_com_default_printed && $rating_style === 'default' && $platform_name !== 'facebook' && $platform_name !== 'booking.com') { ?>
    ...
    <?php if ($platform_name === 'facebook') { ?>
            ...
            echo ($recommendation_type === 'positive') ? '<span>' . esc_html($recommends) . '</span>' : '<span>' .esc_html($does_not_recommend). '</span>';
            ?>
    <?php } ?>
</div>